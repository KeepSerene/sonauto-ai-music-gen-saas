...
class SongGenServer:
    ...
    def generate_track(self, request: MusicGenerationRequest) -> GeneratedTrackResponse:
        ...
        thumbnail_prompt = (
            f"Abstract atmospheric album cover art, deep bokeh light orbs, "
            f"cinematic color grading inspired by the mood of {request.prompt}, "
            f"soft luminous halos, layered translucent color washes, "
            f"premium editorial music aesthetic, dark vignette edges, "
            f"centered radial composition, no text, no people, no instruments, "
            f"no animals, no faces, no hands"
        )
